  ...
  function BeginEnd(begin, end, pat, rep) {
    ...
    while ((result = reg.exec(code)) !== null) {
      if (!Contain(matches, result[1])) {
        matches.puch(result[1]);
        var temp = result[1].replace(pat, rep);
        fullCode = fullCode.replace(new RegExp(result[1], 'igm'), temp);
      }
    }
    code = fullCode;
  }
  ...
  function BeginNotEnd(begin, end, pat, rep) {
    ...
    while ((result = reg.exec(code)) !== null) {
      if (!Contain(matches, result[0])) {
        matches.puch(result[0]);
        var cut = result[0].replace(regCut, '');;
        var temp = cut.replace(pat, rep);
        fullCode = fullCode.replace(new RegExp(cut, 'igm'), temp);
      }
    }
    code = fullCode;
  }
  ...
  function NotBeginNotEnd(begin, end, pat, rep) {
    ...
    while ((result = reg.exec(code)) !== null) {
      if (!Contain(matches, result[0])) {
        ...
        fullCode = fullCode.replace(new RegExp(result[0], 'igm'), temp);
      }
    }
    code = fullCode;
  }
