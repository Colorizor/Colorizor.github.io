var clz = (function() {
  ...
  function BeginEnd(begin, end, pat, rep) {
    ...
    var regCutBegin = new RegExp(begin, 'igm');
    var regCutEnd = new RegExp(end, 'igm');
    var result,
        matches = [],
        fullCode = code;
    //Colorize
    while ((result = reg.exec(code)) !== null) {
      if (!Contain(matches, result[0])) {
        matches.push(result[0]);
        var cut = result[0].replace(regCutBegin, '').replace(regCutEnd, '');
        var temp = cut.replace(pat, rep);
        fullCode = fullCode.replace(new RegExp(Escape(cut, 'igm')), temp);
      }
    }
    code = fullCode;
  }
  ...
  function NotBeginNotEnd(begin, end, pat, rep) {
    ...
    while ((result = reg.exec(code)) !== null) {
      if (!Contain(matches, result[0])) {
        ...
        fullCode = fullCode.replace(new RegExp(Escape(result[0]), 'igm'), temp);
      }
    }
    code = fullCode;
  }
  ...
  function Custom(pattern, pat, rep) {
    ...
    while ((result = reg.exec(code)) !== null) {
      if (!Contain(matches, result[0])) {
        ...
        matches.push(result[0]);
        var temp = result[0].replace(pat, rep);
        fullCode = fullCode.replace(new RegExp(Escape(result[0]), 'g'), temp);
      }
    }
    code = fullCode;
  }
